...
#        pygame.draw.rect(self.win, self.color, pos, self.width)
    ...
    def nextStep(self):
        self.posx += self.vx
        self.posy += self.vy
        if self.posx > self.winw - self.size or self.posx < self.size:
            self.loop += 1
            self.vx = -self.vx
            self.posx += self.vx
        if self.posy > self.winh - self.size or self.posy < self.size:
            self.loop += 1
            self.vy = -self.vy 
            self.posy += self.vy
        if self.loop > 3:
...
#        pos = self.posx, self.posy, self.size, self.size
#        pygame.draw.rect(self.win, self.color, pos, self.width)
        pygame.draw.circle(self.win, self.color, (self.posx, self.posy), self.size, self.width)
