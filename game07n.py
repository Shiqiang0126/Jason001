...
class dLeaf():
    def __init__(self, winid, step=10, color=(255,255,255), lwidth=1):
        self.win = winid
        self.winh, self.winw = winid.get_height()-lwidth, winid.get_width()-lwidth
        self.step = step
        self.x, self.y = 0, (self.step/self.winw)*self.winh
        self.color = color
    ...
    def nextLine(self):
        if self.slp > 0:
            pygame.draw.line(self.win,
                          self.color,
                          (self.x,0),
                          (self.winw,int(self.y)),
                          self.width)
            pygame.draw.line(self.win,
                          self.color,
                          (self.winw - self.x,self.winh),
                          (0,int(self.winh - self.y)),
                          self.width)
            #pygame.draw.line(self.win,
            #             self.color,
            #             (self.winw - self.x,0),
            ...
            self.x += self.step
            self.y = round((self.x/self.winw)*self.winh)
            if self.x > self.winw or self.y > self.winh:
                self.slp = -30 # pause time
